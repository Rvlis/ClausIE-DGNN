...
class ModelHandler:
    def __init__(self, config):
        ...
        self.stopper = EarlyStopping(
            ...
            # ),
            # os.path.join(
            #     "/home/rvlis/project/GraphOIE/graph4nlp-master", self.config["out_dir"], Constants._SAVED_WEIGHTS_FILE
            # ),
            os.path.join(
                "/home/rvlis/project/GraphOIE/GraphOIE", self.config["out_dir"], Constants._SAVED_WEIGHTS_FILE
            ),
    ...
    @torch.no_grad()
    def translate(self, type):
        self.model.eval()
        raw_contents = list()
        if type == 0:
            while True:
                sentence = input("Input single sentence, 'quit' to quit\n")
                if sentence == "quit":
                    break
                raw_contents.append(sentence)
        elif type == 1:
            input_path = input("please input input file path (absolute or relative)")
            with open(input_path) as rf:
                for sentence in rf.readlines():
                    raw_contents.append(sentence.strip())
...
def get_args():
    ...
    parser.add_argument("--type", type=int, choices=[0, 1], default=0, help="input type: 0 is single input; 1 is batch input")
...
def main(config):
    ...
    test_scores = runner.translate(config["type"])
